...
xmax=tft.height();
//Draw face
 tft.fillCircle(ymax-120, xmax-120, 100, YELLOW); //head
 tft.fillCircle(ymax-118, xmax-120, 80, BLACK);//mouth
 tft.fillCircle(ymax-112, xmax-120, 81, YELLOW);//mouth
 tft.fillCircle(ymax-90, xmax-150, 20, BLACK);//left eye
 tft.fillCircle(ymax-90, xmax-90, 20, BLACK);//right eye
//    tft.setRotation(tft.getRotation()+2);
...
for (i=0;i<4;i++)
{
progress(ymax-240, xmax-45, (isBlink?BLACK:WHITE));
...
for (i=0;i<9;i++)
{
progress(ymax-240,xmax- 10, (isBlink?BLACK:WHITE));
isBlink?openeye():closerighteye();
isBlink=!isBlink;
//progress(240, 45,BLACK);
//openeye();
...
{int i, increment;
 ...
 Serial.print("increment*seconds+1:");Serial.println(increment*seconds+1);
  tft.drawRect(ymax-0, xmax-279, increment*seconds+2,22 , WHITE);
  //tft.drawRect(1, 280, increment*seconds,20 , WHITE);
  for (i=0; i<seconds; i++)
  {
tft.fillRect( i*increment+1,xmax-280, increment,20, color); 
delay(1000);
}
}
void closerighteye()
{//close eye
  tft.fillCircle(ymax-85, xmax-90, 20, YELLOW);//right eye blink
