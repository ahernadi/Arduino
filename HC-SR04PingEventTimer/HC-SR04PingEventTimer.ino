...
#define TIMER_US 1000000                                   // 50 uS timer duration 
...
void loop()
{
    ...
    delay(1000);                                       // every 1 S
     Serial.print("distance ");
    Serial.println(echo_duration/57);
}
...
//        Serial.println(state);
      ...
      {                                              // Time out - Initiate trigger pulse
         ...
         state = 1; 
Serial.println("trigger high");         // Changing to state 1 initiates a pulse
      }
...
void distance_flasher()
{
      ...
      {                                                // Flash timer time out
         ...
         {
           range_flasher_counter = echo_duration * 20;  // Reload the timer with the current echo duration
         }
         else
         {
           range_flasher_counter = 25000;              // If out of range use a default
         }
         //if (echo_duration/57<255 and echo_duration/57>0) analogWrite(onBoardLED,echo_duration/57);
         //else 
         digitalWrite( onBoardLED, digitalRead( onBoardLED ) ^ 1 );   // Toggle the onboard LED
         digitalWrite( 10, digitalRead( onBoardLED ) ^ 1 );
      }
}
